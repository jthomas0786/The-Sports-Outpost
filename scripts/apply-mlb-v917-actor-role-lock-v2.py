...
old="const playerImg=id=>id?`https://img.mlbstatic.com/mlb-photos/image/upload/w_180,q_auto:best/v1/people/${encodeURIComponent(id)}/headshot/67/current`:'./images/player-placeholder.png';"
new="const playerImg=id=>/^\\d+$/.test(String(id||''))?`https://img.mlbstatic.com/mlb-photos/image/upload/w_180,q_auto:best/v1/people/${encodeURIComponent(id)}/headshot/67/current`:'./images/player-placeholder.png';"
if old not in s: raise SystemExit('playerImg marker missing')
s=s.replace(old,new,1)

between('function onDeckPerson(','function fielders(',"""function onDeckPerson(feed,side,batterId,preferLive=true){
...
between('function fielders(','function teamInfo(',"""function fielders(feed,side,preferLive=true){
...
between('function currentState(','function cardHTML(',"""function playSides(feed,play){
...
between('async function animateRunnerMoves(','function outBaseSequence(',"""const BASE_CIRCUIT=['home','first','second','third','home'];
...
for old,new,label in [
 ('conceptV914,conceptV915,conceptV916] = await Promise.all([','conceptV914,conceptV915,conceptV916,conceptV917] = await Promise.all([','router destructure'),
 ("        import('./mlb/playstage-v901.js?v=90.42')","        import('./mlb/playstage-v901.js?v=90.43')",'core cache'),
 ("        import('./mlb/playstage-concept-v916.js?v=91.60')","        import('./mlb/playstage-concept-v916.js?v=91.60'),\n        import('./mlb/playstage-concept-v917.js?v=91.70')",'v917 import'),
 ('      conceptV916.installMlbPlaystageConceptV916?.();','      conceptV916.installMlbPlaystageConceptV916?.();\n      conceptV917.installMlbPlaystageConceptV917?.();','v917 install')]:
    if old not in r: raise SystemExit(f'{label} marker missing')
    r=r.replace(old,new,1)
router.write_text(r)

index=Path('index.html')
i=index.read_text();i,n=re.subn(r'\./sports/router\.js\?v=[A-Za-z0-9._-]+','./sports/router.js?v=90.43',i,count=1)
