...
MOBILE = r'''const STYLE_ID='tso-mlb-playstage-concept-v920-mobile-style';
...
mobile=Path('sports/mlb/playstage-concept-v920-mobile.js')
mobile.write_text(MOBILE)

router=Path('sports/router.js')
r=router.read_text()
if "playstage-concept-v920-mobile.js" not in r:
    r,n=re.subn(r'(conceptV916,conceptV917)(\] = await Promise\.all\(\[)',r'\1,conceptV920\2',r,count=1)
    if n!=1: raise SystemExit('router destructure marker missing')
    needle="        import('./mlb/playstage-concept-v917.js?v=91.72')"
    if needle not in r: raise SystemExit('v917 import marker missing')
    r=r.replace(needle,needle+",\n        import('./mlb/playstage-concept-v920-mobile.js?v=92.00')",1)
    needle2='      conceptV917.installMlbPlaystageConceptV917?.();'
    if needle2 not in r: raise SystemExit('v917 install marker missing')
    r=r.replace(needle2,needle2+'\n      conceptV920.installMlbPlaystageConceptV920Mobile?.();',1)
router.write_text(r)

index=Path('index.html')
i=index.read_text()
i2,n=re.subn(r'\./sports/router\.js\?v=[A-Za-z0-9._-]+','./sports/router.js?v=90.46',i,count=1)
if n!=1: raise SystemExit('index router import missing')
index.write_text(i2)

# Keep existing MLB cache assertions current without changing NFL/NHL production imports.
for p in Path('scripts').glob('*selftest*.mjs'):
    t=p.read_text()
    if './sports/router.js?v=90.45' in t:
        p.write_text(t.replace('./sports/router.js?v=90.45','./sports/router.js?v=90.46'))

print('Applied MLB v920 mobile Gamecast layout')
