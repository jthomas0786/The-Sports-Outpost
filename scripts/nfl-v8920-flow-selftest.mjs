...
const warmBoard={...board,state:{...board.state,clockMin:7.5}};
const warmGame={status:'pre',away:{score:0},home:{score:0},liveScore:{status:'in',period:2,clockMin:7.4,lastFetchedAt:now,awayScore:7,homeScore:10}};
assert.equal(halftimeBoardCurrent(warmBoard,warmGame,now),true,'fresh late-Q2 board is usable before halftime even when outer slate status/score lag');
assert.equal(halftimeBoardCurrent(warmBoard,{...warmGame,liveScore:{...warmGame.liveScore,awayScore:14}},now),false,'late-Q2 live score change expires board until refresh');
