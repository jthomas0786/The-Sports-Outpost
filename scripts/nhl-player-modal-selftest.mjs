...
assert.equal(typeof __NHL_PLAYER_MODAL_V913_TEST__.sportsbookPending,'function');
...
const js=fs.readFileSync('sports/nhl/player-modal-v911.js','utf8'),parity=fs.readFileSync('sports/nhl/player-modal-v912.js','utf8'),guard=fs.readFileSync('sports/nhl/player-modal-v913.js','utf8'),css=fs.readFileSync('sports/nhl/player-modal-v912.css','utf8'),wide=fs.readFileSync('sports/nhl/player-modal-v913.css','utf8'),visibility=fs.readFileSync('sports/nhl/player-modal-visibility-v910.css','utf8'),wrapper=fs.readFileSync('sports/nhl/view-v906.js','utf8'),router=fs.readFileSync('sports/router.js','utf8'),researchScript=fs.readFileSync('scripts/nhl-research.mjs','utf8');
...
for(const marker of ['installNhlPlayerModalV912','pendingProbability','sportsbookPending','SPORTSBOOK','tso-nhl-player-card-v913'])assert.ok(guard.includes(marker),`missing NHL v90.13 pending guard: ${marker}`);
...
for(const marker of ["@import url('./player-modal-v912.css?v=90.12')",'position:absolute!important','left:0!important','right:0!important','bottom:0!important','width:auto!important','max-width:none!important'])assert.ok(wide.includes(marker),`missing v90.13 full-column bar lock: ${marker}`);
...
assert.ok(wrapper.includes("./player-modal-v913.js?v=90.13"));
assert.ok(wrapper.includes('player-modal-visibility-v910.css?v=90.10'));
assert.ok(router.includes("./nhl/view-v906.js?v=90.13"));
console.log('NHL player modal: NFL sizing, truly full-width recent bars, pending-state guards, functional slip CTA and mobile layout passed');
