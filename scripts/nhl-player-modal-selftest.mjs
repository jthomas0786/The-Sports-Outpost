...
import {buildNhlPlayerContext,__NHL_PLAYER_MODAL_V909_TEST__} from '../sports/nhl/player-modal-v909.js';
...
const research={players:{p1:{season:2026,games:82,rates:{goals:.4,sog:3.2,points:.9,assists:.5,blocks:.7},shootingPct:.125,recentGames}}};
...
assert.deepEqual(__NHL_PLAYER_MODAL_V909_TEST__.marketsFor(player),['atg','sog','points','assists','blocks']);
assert.deepEqual(__NHL_PLAYER_MODAL_V909_TEST__.marketsFor({...player,position:'G'}),['saves']);
const buckets=__NHL_PLAYER_MODAL_V909_TEST__.buckets(ctx);assert.equal(buckets.length,3);assert.equal(buckets[1].lo,1);
const h2h=__NHL_PLAYER_MODAL_V909_TEST__.rangeRows(ctx,game,player,{range:'h2h',venue:'all'});assert.equal(h2h.length,2,'H2H filter must keep only the current opponent');
const home=__NHL_PLAYER_MODAL_V909_TEST__.rangeRows(ctx,game,player,{range:'10',venue:'home'});assert.equal(home.length,2,'venue filter must use verified homeAway values');
