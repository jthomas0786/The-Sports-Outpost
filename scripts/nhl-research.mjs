...
// Recent completed games power the NFL-style modal charts. Refresh them less often than the live slate because live/current-game production already comes from the summary feed.
const includeCurrent=(slate.games||[]).some(g=>Number(g.seasonType)===2)||new Date(now).getUTCMonth()>=9;
const recentIds=ids.filter(id=>now-(cache.recentChecked[id]||0)>=3*HOUR);
...
await Promise.all(Array.from({length:8},async()=>{while(recentIndex<recentIds.length){const id=recentIds[recentIndex++],refs=[],seen=new Set();let anyLog=false,tempFailure=false;
 for(const season of (includeCurrent?[currentSeason,priorSeason]:[priorSeason])){
  try{
   ...
   // ESPN ordering can vary by sport. Sample both ends, then sort by the fetched game date later.
   for(const item of [...played.slice(0,12),...played.slice(-12)]){
...
for(const [id,refs] of refsByPlayer){
 const rows=refs.map(ref=>recentGameFromSummary(summaries.get(ref.eventId),{playerId:id,teamId:ref.teamId,season:ref.season,eventId:ref.eventId})).filter(Boolean).sort((a,b)=>Date.parse(b.date||0)-Date.parse(a.date||0)).slice(0,10);
 if(rows.length){
