import {lineupReady,lineupState} from '../sports/nhl/lineups.js';
...
const games=slate.games.map(g=>{
 ...
 if(!lineupReady(g))return {gameId:g.id,ready:false,reason:'Fresh confirmed lineups and goalies required'};
 const parts=String(g.clock||'20:00').split(':').map(Number),clockSeconds=parts[0]*60+(parts[1]||0);
 const result=simulateFullGame({players:attachPriors(g,research).map(p=>({...p,confirmedStarter:g.status==='in'?p.currentGoalie:p.confirmedStarter})),status:g.status,period:g.status==='pre'?1:g.period,clockSeconds,seasonType:g.seasonType,lineupsConfirmed:g.lineupsConfirmed===true,currentScore:{[g.away.abbr]:g.away.score,[g.home.abbr]:g.home.score},seed:Number(g.id)});
 const {worlds,overtime,shootouts,...publicResult}=result;return {gameId:g.id,state:lineupState(g),generatedAt:new Date().toISOString(),...publicResult};
