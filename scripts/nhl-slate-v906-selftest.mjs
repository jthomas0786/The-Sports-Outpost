...
assert.match(html,/Away Goalie/,'expanded pool must include goalie markets');
assert.match(html,/Home Goalie/,'expanded pool must include both team goalie markets');
assert.match(html,/data-grade="A\+"/,'NFL-parity grade helper should be used on featured players');
const awayTop=html.split('hk-slate-top-five')[1].split('</section>')[0];
assert.equal((awayTop.match(/hk-slate-player "/g)||[]).length,5,'exactly five featured skaters should be rendered for the away team');
...
console.log('NHL v90.6 slate: one-column matchup, top-five threats, full player pools and goalie expansion passed');
