...
const voidLikeText=value=>/\b(?:void|cancel(?:led|ed)?|abandon(?:ed)?|postpon(?:ed)?|forfeit(?:ed)?|walkover|w\/o|retir(?:ed|ement)|default|disqualif(?:ied|ication)|no\s*contest)\b/i.test(String(value||''));
...
function rawScore(row,side){
  const score=row?.score||row?.scores||row?.result||{};
...
function rawWinner(row){
  const winner=first(row?.winner_team,row?.winnerTeam,row?.winning_team,row?.winningTeam,row?.winner_name,row?.winnerName,row?.winner?.name,row?.winner);
...
async function fetchJson(url,{attempts=3}={}){
  ...
  for(let attempt=1;attempt<=attempts;attempt++){
    ...
    try{
      ...
      if(res.ok){
        return {data:text?JSON.parse(text):null,credits:Number(res.headers.get('x-requests-last')||0)||0};
      }
...
function normalizeEvent(row,sportKey,game){
  ...
  const start=iso(first(row?.commence_time,row?.commenceTime,row?.start_time,row?.startTime,row?.scheduled_at,row?.scheduledAt,row?.date));
...
function normalizeMoneylines(payload,sportKey,game,now){
  ...
  for(const event of arrayPayload(payload)){
    const base=normalizeEvent(event,sportKey,game);if(!base)continue;
    if(Date.parse(base.commenceTime)<=now)continue;
    for(const book of Array.isArray(event?.bookmakers)?event.bookmakers:[]){
      const bookKey=String(book?.key||book?.bookmaker||'').toLowerCase();
...
function normalizeResult(row,sportKey,game){
  ...
  const start=iso(first(row?.commence_time,row?.commenceTime,row?.start_time,row?.startTime,row?.scheduled_at,row?.scheduledAt,row?.match_time,row?.matchTime,row?.date));
  ...
  const voidLike=voidLikeText(statusText)||voidLikeText(row?.reason)||row?.void===true||row?.cancelled===true||row?.canceled===true;
  const homeScore=rawScore(row,'home'),awayScore=rawScore(row,'away');
  let winner=rawWinner(row);
  if(winner&&![home,away].some(team=>norm(team)===norm(winner)))winner='';
  const explicitFinal=row?.completed===true||row?.complete===true||row?.is_final===true||row?.final===true||/\b(?:final|completed|complete|finished|closed)\b/i.test(statusText);
  if(!winner&&!voidLike&&explicitFinal&&homeScore!==null&&awayScore!==null&&homeScore!==awayScore)winner=homeScore>awayScore?home:away;
  const final=Boolean(voidLike||winner||explicitFinal);
  if(!final)return null;
  return {
    id:readId(row,sportKey,home,away,start),sport:'ESPORTS',sportKey,game,commenceTime:start,homeTeam:home,awayTeam:away,
    homeScore,awayScore,status:'FINAL',resultStatus:statusText||'FINAL',final:true,voidLike,winner:winner||null,
...
function cachedFinalMap(snapshot,now){
  ...
  for(const match of Object.values(snapshot?.matches||{})){
    ...
    if(!Number.isFinite(t)||now-t>RESULT_CACHE_MS||t-now>6*3600_000)continue;
...
async function main(){
  const now=Date.now();
  const previousResults=await readJson(RESULTS_FILE,{matches:{}});
  const finals=cachedFinalMap(previousResults,now);
  const moneylineRows=[];
  const diagnostics=[];
  let totalCredits=0;
  ...
  for(const sport of SPORTS){
    ...
    let events=[];
    try{
      const response=await fetchJson(eventUrl,{attempts:2});totalCredits+=response.credits;
      events=arrayPayload(response.data).map(row=>normalizeEvent(row,sport.key,sport.game)).filter(Boolean);
    }catch(error){
      diagnostics.push({sportKey:sport.key,game:sport.game,eventCount:0,oddsCalled:false,resultDates:[],error:`events: ${error?.message||error}`});
      continue;
    }

    const future=events.filter(event=>Date.parse(event.commenceTime)>now);
    let oddsCalled=false,oddsRows=0;
    if(future.length){
      try{
        ...
        console.error(`::warning::${sport.key} Pinnacle h2h failed: ${error?.message||error}`);
      }
    }

    const resultDates=[];let rawResultRows=0,acceptedFinals=0,sampleResultKeys=[];
    if(sport.results){
      ...
      for(const date of dates.slice(0,2)){
        try{
          ...
          if(!sampleResultKeys.length&&raw[0]&&typeof raw[0]==='object')sampleResultKeys=Object.keys(raw[0]).slice(0,30);
          ...
          console.error(`::warning::${sport.key} results ${date} failed: ${error?.message||error}`);
        }
      }
    }
    diagnostics.push({sportKey:sport.key,game:sport.game,eventCount:events.length,futureEvents:future.length,oddsCalled,oddsRows,resultDates,rawResultRows,acceptedFinals,sampleResultKeys});
  }
