...
assert.ok(!nhlCss.includes('76px'),'NHL parity layer must not reintroduce the old L5 76px cap');
assert.ok(!nhlCss.includes('58px'),'NHL parity layer must not reintroduce the old mobile 58px cap');
assert.ok(!nhlCss.includes('46px'),'NHL parity layer must not reintroduce the old compact 46px cap');
...
assert.ok(router.includes("./mlb/player-modal-parity-v901.js?v=90.1"),'Router must load MLB NFL-parity modal');
assert.ok(router.includes('installMlbPlayerModalParityV901'),'Router must install MLB modal parity enhancer');

console.log('Player modal parity: NFL remains source of truth; MLB uses NFL header/prop/chart geometry and NHL recent bars fill their full game columns.');
