const STYLE_ID='tso-mlb-live-game-switcher-v902';
...
function ensureStyles(){
 ...
 html[data-sport="mlb"] .tso-mlb-live-switcher select{min-height:36px;min-width:280px;max-width:min(560px,72vw);padding:0 34px 0 10px;border:1px solid rgba(66,126,197,.34);border-radius:8px;background:#071a34;color:#edf6ff;font:800 10px 'Space Mono','JetBrains Mono',monospace;cursor:pointer}
 ...
 html[data-sport="mlb"] .tso-mlb-live-switcher.is-idle .tso-mlb-live-switcher-state{color:#9a8484}
...
function itemFromCard(card){
 ...
 const badge=String(card.querySelector('.slate-badge')?.textContent||'').replace('●','').trim();
 const detail=String(card.querySelector('.slate-time')?.textContent||'').trim();
 const live=card.classList.contains('is-live')||/^live$/i.test(badge);
 const final=/final/i.test(badge);
 const suffix=live?(detail||'LIVE'):final?'Final':(detail||badge||'Scheduled');
 return {id,live,final,label:`${away} @ ${home} · ${suffix}`};
}
function collect(){
 const list=document.querySelector('html[data-sport="mlb"] .slate-list');
 const cards=[...document.querySelectorAll('html[data-sport="mlb"] .slate-card[data-gid]')];
 if(list)slateGames.clear();
 for(const card of cards){const item=itemFromCard(card);if(item.id)slateGames.set(item.id,item);}
 return [...slateGames.values()];
}
...
function switchGame(id){
 ...
 const card=document.querySelector(`html[data-sport="mlb"] .slate-card[data-gid="${CSS.escape(id)}"]`);
 if(!card)return;
...
function render(){
 ...
 if(list&&games.length){
  ...
  const selected=games.some(g=>g.id===currentGameId)?currentGameId:(preferred(games)?.id||'');
  host.classList.toggle('is-idle',!games.some(g=>g.live));host.innerHTML=markup(games,selected);wire(host);
 ...
 if(modal&&games.length&&currentGameId&&games.some(g=>g.id===currentGameId)){
  ...
  host.classList.toggle('is-idle',!games.some(g=>g.live));host.innerHTML=markup(games,currentGameId);wire(host);
...
export function installMlbLiveGameSwitcherV901(){
 ...
 installed=true;
 document.addEventListener('click',e=>{const card=e.target?.closest?.('html[data-sport="mlb"] .slate-card[data-gid]');if(card){const item=itemFromCard(card);if(item.id){currentGameId=item.id;slateGames.set(item.id,item);setTimeout(queue,0);}}},true);
...
export const __MLB_LIVE_SWITCHER_V901_TEST__={itemFromCard,switchGame,collect,preferred};
