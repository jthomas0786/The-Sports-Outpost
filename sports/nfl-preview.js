import { mountOrUpdateNflPlaystageV886B, renderNflPlaystageV886BHTML } from './nfl/playstage-v886b.js?v=88.6b';
import { ensureNflPlaystageV886BStyles } from './nfl/gamecast-v886b-styles.js?v=88.6b';
  ...
  try{
    ...
    if(!NFL_DEMO_MODE) startLivePolling(d,()=>{
      ...
      if(state.game && state.gamecastTab==='game'){
        ...
        if(current && root.querySelector('[data-tso-v886b-gamecast]')){ requestAnimationFrame(()=>render()); return; }
...
function gamecastDashboardHTML(g,p,{embedded=false,tab=null}={}){
  ...
  const gameView=renderNflPlaystageV886BHTML(g,{halftime:state.halftime});
...
export async function mount(){
  ensureNflGamecastConceptStyles(); ensureNflLaunchStyles(); ensureHalftimeLabStyles(); ensureNflGamecastV883aStyles(); ensureNflGamecastV883bStyles(); ensureNflGamecastV884Styles(); ensureNflPlaystageV886BStyles();
...
function __tsoV885MountFromDom(){
  ...
  try{
    ...
    if(!g) return;
    mountOrUpdateNflPlaystageV886B(__tsoV885Root(), g, { halftime: typeof state!=='undefined' ? state?.halftime : null });
