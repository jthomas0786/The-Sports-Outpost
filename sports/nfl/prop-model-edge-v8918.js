...
const STRIP='tso-nfl-prop-model-v8918';
...
async function loadSim({force=false}={}){
  ...
  loading=(async()=>{
    ...
    }catch(e){console.warn('[NFL Prop Model v89.18] simulation cache unavailable:',e);return simIndex;}
...
function twoPlusFromPlayer(playerSim){
  const atd=finite(playerSim?.probabilities?.atd);
  const two=finite(playerSim?.probabilities?.twoPlusTd);
  if(two==null)return null;
  return Math.max(0,Math.min(atd==null?1:atd,two));
}
function stripHtml(view,playerSim,market){
  ...
  const two=market==='atd'?twoPlusFromPlayer(playerSim):null;
  const multi=two==null?'':`<span class="two-plus" data-tso-two-plus-td="${two.toFixed(4)}" title="Probability of this player scoring at least two rushing/receiving touchdowns in the same correlated simulation worlds">2+ TD <b>${pct(two)}</b></span>`;
  return `<div class="${STRIP}__main"><strong>TSO ${pct(view.tsoProbability)}</strong>${multi}${projection}${edgeText}<b>${esc(view.tier)}</b></div><div class="${STRIP}__sub"><small>${esc(iterText(view.iterations))} · ${esc(basis)} · ${esc(view.probabilityMethod)}</small>${range}</div>`;
}
function patchCard(card){
  ...
  const two=market==='atd'?twoPlusFromPlayer(row.playerSim):null;
  const sig=[market,side,line??'',price??'',row.playerSim?.playerId||row.playerSim?.espnId||'',row.iterations,view.tsoProbability,view.edgePoints,two??''].join('|');
  let strip=card.querySelector(`:scope > .${STRIP}`);
  if(strip?.dataset.sig===sig)return;
  card.querySelector(':scope > .tso-nfl-prop-model-v8917')?.remove();
  if(!strip){strip=document.createElement('div');strip.className=STRIP;strip.setAttribute('aria-label','TSO simulation model including 2+ touchdown probability');card.append(strip);}
  strip.dataset.sig=sig;strip.innerHTML=stripHtml(view,row.playerSim,market);
}
function patchAll(){
...
function ensureStyle(){
  if(document.getElementById('tso-nfl-prop-model-v8918-style'))return;
  const s=document.createElement('style');s.id='tso-nfl-prop-model-v8918-style';s.textContent=`
    ...
    #nflView .${STRIP}__main .two-plus{display:inline-flex;align-items:center;gap:5px;padding:4px 7px;border:1px solid rgba(245,158,11,.42);border-radius:999px;background:rgba(245,158,11,.11);color:#fbbf24}
    #nflView .${STRIP}__main .two-plus b{font:950 10px 'JetBrains Mono',monospace;color:#fff;text-transform:none;letter-spacing:0}
    ...
    @media(max-width:760px){#nflView .${STRIP}{align-items:flex-start;flex-direction:column;gap:4px}#nflView .${STRIP}__sub{justify-content:flex-start}#nflView .${STRIP}__main .two-plus{padding:3px 6px}}
  `;document.head.append(s);
}

export async function installNflPropModelEdgeV8918(){
...
export async function refreshNflPropModelEdgeV8918(){await loadSim({force:true});schedule();}
export const __V8918_UI_TEST__={buildIndex,marketFromCard,sideLineFromCard,priceNumber,iterText,norm,twoPlusFromPlayer,stripHtml};
