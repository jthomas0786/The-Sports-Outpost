// NHL v90.6 slate enhancement: NFL-style single-column matchups with team player pools.
...
function ensureStyle(){
 if(typeof document==='undefined'||document.getElementById('nhl-slate-v906'))return;
 const link=document.createElement('link');link.id='nhl-slate-v906';link.rel='stylesheet';link.href='./sports/nhl/slate-v906.css?v=90.6';document.head.appendChild(link);
}
...
function teamBoard(game,team){
 const all=teamPlayers(game,team),ranked=rankNhlSlatePlayers(all,game.id,research,sim),top=ranked.slice(0,5);
 return `<section class="hk-slate-team-board"><div class="hk-slate-team-head">${teamLogo(team)}<div><b>${esc(team.name)}</b><span>Top Goal Threats · ${ranked.length} skaters</span></div></div><div class="hk-slate-team-cols"><span>Player</span><span>Model / baseline</span><span>Grade</span></div><div class="hk-slate-top-five">${top.map(p=>playerRow(p,game)).join('')||'<div class="hk-slate-no-players">Player pool is loading.</div>'}</div></section>`;
}
function fullTeamPool(game,team){
 const all=teamPlayers(game,team),skaters=rankNhlSlatePlayers(all,game.id,research,sim),goalies=all.filter(p=>p.position==='G'&&p.active!==false).sort((a,b)=>Number(b.confirmedStarter)-Number(a.confirmedStarter)||String(a.name||'').localeCompare(String(b.name||'')));
 return `<section class="hk-slate-full-team"><div class="hk-slate-full-head">${teamLogo(team)}<div><b>${esc(team.abbr)}</b><span>${skaters.length} skaters · ${goalies.length} goalies</span></div></div><div class="hk-slate-full-list">${[...skaters,...goalies].map(p=>playerRow(p,game,{compact:true})).join('')||'<div class="hk-slate-no-players">No eligible players reported.</div>'}</div></section>`;
}
export function renderNhlSlatePoolsHTML(game,{research:researchDoc=null,sim:simDoc=null,gradeForLean=null,gradeRingHTML=null}={}){
 ...
 const html=`<div class="hk-slate-matchup-pools" role="region" aria-label="${esc(game.away?.name)} at ${esc(game.home?.name)} player pools"><div class="hk-slate-featured-grid">${teamBoard(game,game.away)}${teamBoard(game,game.home)}</div><details class="hk-slate-full-pools"><summary><span class="closed-label">View Full Player Pools</span><span class="open-label">Hide Full Player Pools</span><i>⌄</i></summary><div class="hk-slate-full-grid">${fullTeamPool(game,game.away)}${fullTeamPool(game,game.home)}</div></details></div>`;
 research=priorResearch;sim=priorSim;helpers=priorHelpers;return html;
}
async function getJSON(path){const r=await fetch(`${path}${path.includes('?')?'&':'?'}v=90.6-${Date.now()}`,{cache:'no-store'});if(!r.ok)throw new Error(`HTTP ${r.status}`);return r.json();}
...
function wirePool(pool){
 ...
 pool.addEventListener('click',e=>e.stopPropagation());
...
function enhance(){
 ...
 for(const card of host.querySelectorAll('.hk-matchup[data-hk-game]')){
  ...
  const pool=wrap.firstElementChild;pool.dataset.stamp=dataStamp;
