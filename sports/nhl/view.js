...
function live(){
 ...
 const plays=g.summaryUnavailable?empty('Play-by-play is temporarily unavailable.'):g.plays.map(p=>`<div class="hk-play"><small>P${p.period} · ${esc(p.clock)}</small><p>${esc(p.text)}</p></div>`).join('')||empty('Waiting for the first reported play.');
 return `<div class="hk-live-toolbar"><label>Game<select id="hk-game">${games.map(x=>`<option value="${esc(x.id)}" ${x.id===g.id?'selected':''}>${esc(x.away.abbr)} @ ${esc(x.home.abbr)}</option>`).join('')}</select></label><span>${g.status==='in'&&freshGame(g)?'● LIVE · auto-refreshing':esc(time(g))}</span></div>${scorebar(g)}${g.status==='pre'?empty('Gamecast opens at puck drop. Lineup and goalie confirmation appear here as they become available.'):`<div class="hk-live-grid"><section class="hk-live-panel"><div class="hk-panel-head"><span>Play by Play</span><small>Newest events update automatically</small></div><div class="hk-play-list">${plays}</div></section><section class="hk-live-panel hk-box-panel"><div class="hk-panel-head"><span>Box Score</span><small>${esc(score(g))}</small></div><div class="hk-table"><table><thead><tr><th>Player</th><th>G</th><th>A</th><th>SOG</th><th>BLK</th><th>SV</th><th>TOI</th></tr></thead><tbody>${g.players.map(p=>`<tr><th>${esc(p.name)} <small>${esc(p.team)}</small></th>${['goals','assists','sog','blocks','saves','toi'].map(k=>`<td>${esc(p.current?.[k]??'—')}</td>`).join('')}</tr>`).join('')}</tbody></table></div></section></div>`}`;
}
...
document.addEventListener('click',e=>{const b=e.target.closest('[data-hk-game]');if(!b)return;gameId=b.dataset.hkGame;window.closeCommandCenter?.();window.DW_openNhlTab('live');});
document.addEventListener('keydown',e=>{const b=e.target.closest?.('[data-hk-game]');if(!b||!['Enter',' '].includes(e.key))return;e.preventDefault();gameId=b.dataset.hkGame;window.DW_openNhlTab('live');});
