...
function ensureConnectorStyle(){
  ...
  style.id=STYLE_ID;
  style.textContent='#ppLineCheck{display:none!important}.pps-tools{justify-content:flex-end!important}.pps-tools .pps-tool{flex:0 0 174px}@media(max-width:760px){.pps-tools{justify-content:stretch!important}.pps-tools .pps-tool{flex:1 1 auto}}';
  document.head.appendChild(style);
}
function simplifyConnector(root){
  root.querySelector('#ppLineCheck')?.remove();
  const tools=root.querySelector('.pps-tools');
  if(tools){tools.style.justifyContent='flex-end';}
}
function polishShare(root){
  root.querySelectorAll('.pps-share[data-share]').forEach(btn=>{
    ...
    const el=holder.querySelector('svg');if(el){el.style.width='24px';el.style.height='24px';el.style.stroke=kind==='copy'?'currentColor':'none';el.style.strokeWidth='1.8';el.style.strokeLinecap='round';el.style.strokeLinejoin='round';}
  });
}
...
function polishHeadshots(root){
  ...
  root.querySelectorAll('.pps-leg').forEach(row=>{
    ...
    const fallback=initialsAvatar(playerOf(leg));
    const photo=directPhoto(leg)||espnPhoto(leg);
    img.style.visibility='visible';
    if(img.dataset.ppLeg!==String(leg.id||playerOf(leg))){img.dataset.ppLeg=String(leg.id||playerOf(leg));img.src=fallback;}
    if(photo&&img.dataset.ppPhoto!==photo){
      img.dataset.ppPhoto=photo;
      const probe=new Image();
      probe.onload=()=>{if(img.isConnected)img.src=photo;};
      probe.onerror=()=>{if(img.isConnected)img.src=fallback;};
      probe.src=photo;
    }else if(!photo)img.src=fallback;
  });
}
function polish(){const root=document.getElementById(ROOT_ID);if(!root)return;ensureConnectorStyle();simplifyConnector(root);polishShare(root);polishHeadshots(root);}
function arm(){
