...
async function swapView(active) {
  ...
  if(active === 'nhl'){
    try { await (await import('./nhl/view-v906.js?v=90.18&props=1&slate=2')).mount(); }
  ...
  if (active === 'mlb') {
    try {
      ...
      playerBvp.installMlbPlayerModalBvpV902?.();
  ...
  if (active === 'nfl') {
    try {
      const mod = await import('./nfl-preview-v893.js?v=89.54');
      await mod.mount();
      ...
      try {
        const researchUi = await import('./nfl-research-ui.js?v=86.10');
