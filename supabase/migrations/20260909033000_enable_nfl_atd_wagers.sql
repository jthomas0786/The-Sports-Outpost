-- v81 — enable NFL Anytime TD legs in the existing points wagering system.
-- Existing MLB Home Run behavior remains the default/backwards-compatible path.

alter table wager_legs
  add column if not exists sport text not null default 'mlb';

create index if not exists wager_legs_sport_pending_idx
  on wager_legs(sport, game_pk)
...
begin
  ...
  -- Validate every leg before any points move. Tonight's supported markets are
  -- intentionally narrow: MLB Home Run + NFL Anytime TD only.
  ...
  loop
    ...
    v_shaded_prob := (v_leg->>'probability')::numeric;
    if v_shaded_prob is null or v_shaded_prob <= 0 or v_shaded_prob >= 1 then
      raise exception 'invalid probability on a leg';
    end if;

    if v_first_sport is null then
      v_first_sport := v_leg_sport;
      v_wager_sport := v_leg_sport;
    elsif v_first_sport <> v_leg_sport then
      v_wager_sport := 'mixed';
    end if;
  end loop;

  -- Atomic balance check + debit, unchanged from the existing system.
  ...
  -- Server-authoritative TSO point odds, unchanged: probability is shaded by
  -- house_edge and capped by max_payout_multiplier. Sportsbook prices shown in
  -- the UI are informational and are never trusted for point settlement.
  for v_leg in select * from jsonb_array_elements(legs_json)
  loop
